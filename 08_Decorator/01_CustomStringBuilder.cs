...
// decorator around a StringBuilder, decorator usally helps to build
// some sort of multiple inheritance, but in this case is useful
// around a single class because it is sealed so we can inherit directly
// from StringBuilder
namespace Decorator.CustomStringBuilder
{
 public class CodeBuilder
 {
  // here we implement the decorator
  ...
  // all that follows is the implementation of the delagating members
  // where we just implement all the methods that the class we
  // are decorating, this can be generated using ReSharper
  // of course we can customize the behaviour on each method or add
  // new ones to extend the functionality
  ...
  // in some methods with fluent interface where it retunrs
  // the same object we have to call before the method and then
  // return this but as a CodeBuilder, not the original class
